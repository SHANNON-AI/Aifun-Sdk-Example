...
//该实现需要版本PHP7.1，如果PHP低于这个版本可以使用 https://github.com/Spomky-Labs/php-aes-gcm
...
function encrypt($data,$key,$iv,&$tag){
	$algo = 'aes-256-gcm';
	$ciphertext = openssl_encrypt($data, $algo, $key, OPENSSL_ZERO_PADDING, $iv,$tag);
...
function decrypt($ciphertext,$key,$iv,$tag){
	$algo = 'aes-256-gcm';
	$data = openssl_decrypt($ciphertext, $algo, $key, OPENSSL_ZERO_PADDING, $iv,$tag);
	echo "decrypt:	" . $data . "\r\n";
	return $data;
}
...
function test2(){
	...
	$key = "mlMUtKNZL/B1clOS8BFkO7mDuRZPJAMlVmo+vtNzo9Y="; //秘钥(Base64格式)
	$algo = 'aes-256-gcm';
	$iv = random_bytes(openssl_cipher_iv_length($algo));	//随机向量
...
function test3(){
	...
	echo "tag64:	" . $tag64 . "\r\n";
	decrypt($cipher64, base64_decode($key), base64_decode($iv64), base64_decode($tag64));
...
function test4(){
	...
	$tag = NULL;
	$cipher64 = encrypt($data,base64_decode($key),base64_decode($iv64),$tag);
	$tag64 = base64_encode($tag);
	echo "cipher64:	" . $cipher64 . "\r\n";
	echo "tag64:	" . $tag64 . "\r\n";
	//需要把$cipher64 和 $tag64 解码成二进制数据，然后拼接成新的二进制数据，再进行Base64编码得到最终加密数据
	$arrayCipher = unpack("C*", base64_decode($cipher64));
	$arrayTag = unpack("C*", base64_decode($tag64));
